...
-- 8 Feb 2020.
-- Fixes to get_work_summary to improve performance

CREATE INDEX service_historic_service_id_index
    ON application.service_historic USING btree
...
CREATE OR REPLACE FUNCTION application.get_work_summary(
	from_date date,
	to_date date)
...
BEGIN
   ...
   RETURN query 
	 ...
	 AND   ser_hist.rowversion = (SELECT MAX(ser_hist2.rowversion)
					  ...
					  -- 8 Feb 2020: Link on ser rather than ser_hist to improve performance. 
				      WHERE ser.id = ser_hist2.id
					  AND   ser_hist2.status_code IN ('pending', 'lodged')
    ...
    app_changed AS ( -- All applications that changed status during the reporting period
	 ...
	 AND   app_hist.rowversion = (SELECT MAX(app_hist2.rowversion)
				      FROM  application.application_historic app_hist2
					  -- 8 Feb 2020: Link on app rather than app_hist to improve performance. 
				      WHERE app.id = app_hist2.id
					  AND   app_hist2.status_code IN ('lodged', 'requisitioned')
...
COMMENT ON FUNCTION application.get_work_summary(date, date)
    IS 'Returns a summary of the services processed for a specified reporting period. Used by the Lodgement Statistics Report.';
